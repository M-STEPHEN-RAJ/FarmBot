...
import StoreSidebar from "@/app/components/admin/store/StoreSidebar";
...
const SellerStore = () => {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(`${API}/admin/products?${query}`, {
          ...
          {loading
            ...
            : products.map((product) => (
                <div
                  ref={addToRefs}
                  key={product._id}
                  onClick={() => router.push(`/admin/store/${product._id}`)}
                  ...
                  <div className="mt-17 space-y-3">
                    ...
                    <div className="flex gap-3">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          updateStatus(product._id, "blocked");
                        }}
                        className="w-full py-[3px] text-sm text-[#6A1B9A] border border-[#6A1B9A] rounded-md cursor-pointer mt-1"
                      >
                        Block
                      </button>
                      <button
                        ...
                        className="w-full py-[3px] text-sm text-white bg-[#6A1B9A] rounded-md cursor-pointer mt-1"
                      >
                        Allow
                      </button>
                    </div>
